...
serve(async (req: Request) => {
  ...
  try {
    ...
    for (let index = 0; index < leaderboard.length && index < 50; index++) {
      ...
      if (!isEligible) {
        console.warn(`Skipping reward for leaderboard position ${index + 1}: ${reasons.join(', ')}`)
        continue
      }

      // Increment actual rank only when we add a reward (no gaps)
      actualRank += 1
      ...
      rewards.push({
        ...
        wallet_address,
        amount,
        rank: actualRank, // Use actualRank instead of index + 1 to avoid gaps
        score: entry.score,
      })
    }
